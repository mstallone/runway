...
Runway sends `GET https://dev.meta.ai/usage/` with the existing browser session and reads the embedded `subscription_quota_usage` object. Five-hour and weekly percentages are `weighted_used / weighted_limit × 100`; reset times and the source observation time come from the same object. Numeric-only tier IDs are not displayed as plan names.

This follows the dashboard approach in [OpenUsage PR #1248](https://github.com/robinebers/openusage/pull/1248). It is an undocumented page format: missing quota or a changed page produces an explicit unavailable warning, never invented zero usage. Redirects are followed only to the usage page on HTTPS `dev.meta.ai`; a redirect to the homepage or login is treated as an expired session. The request uses no shared cookie jar or disk cache.

Remote reads have a 15-minute minimum interval, including manual refreshes and failures. Concurrent refreshes share one request. A 429 honors `Retry-After` (seconds or HTTP date), with at least the same 15-minute cooldown. Transient failures retain the last reported meters, their original timestamp, and a warning. Logging out, losing access to the browser session, or selecting a new cookie clears the old meters. Local history continues refreshing independently. The request floor/cache are per running app process; relaunching starts a new process.
