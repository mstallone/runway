...
Remote fetches have a 15-minute minimum interval, including manual refreshes and failures. Concurrent refreshes share one fetch sequence. Authentication rejection tries another browser profile; network errors and rate limits stop the sequence. A 429 honors `Retry-After` (seconds or HTTP date), with at least the same 15-minute cooldown. Transient failures retain the last reported meters, their original timestamp, and a warning. Logging out, losing access to the browser session, or selecting a new cookie clears the old meters. Local history continues refreshing independently. The request floor/cache are per running app process; relaunching starts a new process.
