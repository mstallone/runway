...
the same account. The values in Runway therefore describe the account-wide pool, not just this Mac.
...
`KIMI_CODE_HOME` defaults to `~/.kimi-code`. Runway refreshes an expiring access token the same way
...
Like Kimi Code, Runway maps a non-default base URL or OAuth host to the CLI's deterministic
...
3. Refresh Runway. Kimi is detected from the local OAuth credential and turns on automatically
...
show the wrong account or send the key to the wrong service, so Runway deliberately uses the
...
Runway accepts the same response aliases as the CLI and reports malformed responses as errors
