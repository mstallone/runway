...
For supported GPT-5.4, GPT-5.5, and GPT-5.6 models, requests above 272k input tokens use OpenAI's long-context rates for the whole request. Cached input uses the published cache-read discount when the pricing source provides one, otherwise the full input rate. Fast/priority estimates use each model's published Codex multiplier (for example 2.5× for GPT-5.5). Model names ending in `-fast` are normalized to their base rate before that multiplier is applied once.
...
- **Spend tiles show "No data"**: Runway found no Codex session logs in the last 30 days. If your Codex home is custom, set `CODEX_HOME` so both the Codex CLI and Runway look in the same place.
