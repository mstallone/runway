...
final class GrokProvider: ProviderRuntime {
    ...
    var widgetDescriptors: [WidgetDescriptor] {
        [
            ...
            // Local spend tiles, estimated from Grok CLI session activity (see GrokLogUsageScanner).
    ...
    private func probe(state: inout GrokAuthState, accessToken: String) async throws -> ProviderSnapshot {
        ...
        // Local spend tiles, read natively from Grok CLI session activity and priced via the shared
        // pricing store. `scan` is awaited so file discovery + parsing runs off the main actor.
    ...
    private func refreshAccessToken(state: inout GrokAuthState) async -> String? {
        ...
        state.entry.expiresAt = RunwayISO8601.string(from: expiresAt)
