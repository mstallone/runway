...
/// Builds a per-day token/cost series from pi's session logs for one Runway card, so usage that
...
actor PiUsageScanner {
    ...
    /// Parse every mapped assistant usage line of one session file. Lines for pi providers Runway
